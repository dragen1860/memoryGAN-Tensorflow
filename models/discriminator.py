...
def memory_d(model, x, is_training, reuse=False):
	...
	k_dim = model.key_dim # 512
	z_dim = model.z_dim # 16
	...
	with slim.arg_scope(ops_with_bn, is_training=is_training):
		with tf.variable_scope('d_', reuse=reuse) as scope:
			...
			if model.dataset_name == 'fashion':
				...
				h = conv2d(x, f_dim, 3, 1, act=tf.nn.elu, norm=None) # 3 => 128
				h = conv_mean_pool(h, f_dim, 3, act=None, norm=None) # 16x16x128
				h += conv_mean_pool(x, f_dim, 1, act=None, norm=None) # + 16x16x128
				h = tf.nn.elu(ln(h))

				h = residual_block(h, resample='down', act=tf.nn.elu, norm=ln) # 8x8x128
				h = residual_block(h, resample=None, act=tf.nn.elu, norm=ln) # 8x8x128
				h = residual_block(h, resample=None, act=tf.nn.elu, norm=ln) # 8x8x128

				q = conv2d(h, k_dim, 4, 2, act=None, norm=None) # 4x4x512
				q = tf.reduce_mean(q, axis=[1, 2]) # bx512
				q = tf.reshape(q, [bs, -1]) # bx512
