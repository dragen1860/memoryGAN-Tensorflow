...
def train(model, sess):
	...
	print
	"[*] Traing Start : N=%d, Batch=%d, epoch=%d, max_iter=%d" \
	...
	try:
		for idx in xrange(1, max_iter):
			...
			if ((idx * model.batch_size) % N < model.batch_size) and idx > 1:
				...
				print
				'[Epoch %(epoch)d] time: %(total_time)4.4f, d_real: %(d_real).8f, d_fake: %(d_fake).8f, sec_per_epoch: %(sec_per_epoch)4.4f' % locals()
...
def _save_samples(model, sess, epoch):
	...
	print
	"Save Samples at %s/%s" % (model.sample_dir, 'samples_%s' % (epoch))
